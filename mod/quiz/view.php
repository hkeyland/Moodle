...
/*
Humberto Keymur
Genera botón de edición en cuestionarios
Se inserta dentro de navbar
*/

//se pega despùes del header -> echo $OUTPUT->header();

//Obtenemos id de recurso
$resource=split('=', $_SERVER["REQUEST_URI"]);

$editPage="/moodle/course/modedit.php?update=".$resource[1]."&return=1";
$editUrl="/moodle/mod/quiz/edit.php?cmid=".$resource[1];
$bancUrl="http://adistancia.lacienciaentuescuela.edu.mx/moodle/question/edit.php?cmid=".$resource[1];
$prevUrl="/moodle/mod/quiz/startattempt.php?cmid=".$resource[1];

//Usuarios que verán este boton
if($USER->id == 2 || $USER->id == 4){
        echo '<script>
                $(document).ready(function(){
                        //alert("HOLAS");
                        var value = $("[name=sesskey]").val();
                        var prevUrl="/moodle/mod/quiz/startattempt.php?cmid='.$resource[1].'&sesskey="+value;
                        
                        $(".navbar").append( "<form method=\"post\" action=\"'.$editPage.'\"><input value=\"Editar\" type=\"submit\"></form>" );
                        $(".navbar").append( "<form method=\"post\" action=\"'.$editUrl.'\"><input value=\"Editar Preguntas\" type=\"submit\"></form>" );
                        $(".navbar").append( "<form method=\"post\" action=\"'.$bancUrl.'\"><input value=\"Banco de Preguntas\" type=\"submit\"></form>" );
                        $(".navbar").append( "<form method=\"post\" action=\""+prevUrl+"\"><input value=\"Vista previa\" type=\"submit\"></form>" );
                });
        </script>';
}
